...
export async function GET() {
  ...
  const result = await query(
    "SELECT id, name, key_prefix, last_used_at, revoked_at, created_at FROM api_keys WHERE user_id = $1 ORDER BY created_at DESC",
    [user.id],
  );
...
export async function POST(request: Request) {
  ...
  await query(
    "INSERT INTO api_keys(id, user_id, name, key_hash, key_prefix) VALUES ($1, $2, $3, $4, $5)",
    [record.id, record.userId, record.name, record.keyHash, record.keyPrefix],
  );
  await recordAudit(user.id, "api_key.created", "api_key", record.id);
  return NextResponse.json(
    { id: record.id, name: record.name, key: plaintext },
