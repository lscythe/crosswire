...
test("invalid fields cannot hide behind an enabled toggle", async () => {
  const response = await PATCH(
    new Request("http://localhost", {
      method: "PATCH",
      body: JSON.stringify({ enabled: false, visibility: "invalid" }),
    }),
    context,
  );
...
test("connection test returns model IDs without upstream metadata", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn(async () =>
      Response.json({ data: [{ id: "model-a", secret: "saved-secret" }, { invalid: true }] }),
    ),
  );
...
test.each([{}, { apiKey: "" }, { enabled: "false" }, { baseUrl: "http://localhost/v1" }])(
  "rejects invalid edits: %j",
  async (body) => {
    expect(
      (
        await PATCH(
...
test.each([null, { model: 123 }, { model: " " }, { model: "x".repeat(201) }])(
  "probe validates model input: %j",
  async (body) => {
    expect(
      (
        await probe(
...
])("rejects invalid quota settings: %j", async (body) => {
  expect(
    (
      await PATCH(
        new Request("http://localhost", { method: "PATCH", body: JSON.stringify(body) }),
...
test("limited probes return retry information without contacting the provider", async () => {
  vi.mocked(query)
    .mockResolvedValueOnce({ rows: [{ owner_user_id: "owner", visibility: "public" }] } as never)
    .mockResolvedValueOnce({ rows: [{ retry_after: 45 }] } as never);
  const fetchMock = vi.fn();
  vi.stubGlobal("fetch", fetchMock);
  const response = await probe(
    new Request("http://localhost", { method: "POST", body: '{"model":"test"}' }),
    context,
  );
