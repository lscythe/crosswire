...
export async function POST(request: Request) {
  ...
  const user = await withTransaction(async (client) => {
    ...
    const result = await client.query("SELECT id, username, email, name, role, password_hash, disabled_at, must_change_password FROM users WHERE username = $1 FOR UPDATE", [body.data.username]);
    ...
    return { id: row.id, username: row.username, email: row.email, name: row.name, role: row.role, mustChangePassword: row.must_change_password };
  });
