...
vi.mock("../../../../lib/auth", () => ({ hashOpaqueToken: () => "hash", sessionCookie: () => ({ name: "session", value: "token" }) }));
...
const query = vi.fn();
const account = { id: "user", username: "alice", email: "contact@example.com", password_hash: "secret-hash", role: "member", must_change_password: true };
...
it.each([{ email: "alice@example.com", password: "valid-password" }, { username: "alice@example.com", password: "valid-password" }, { username: "ab", password: "valid-password" }, { username: "a".repeat(65), password: "valid-password" }])("rejects malformed login before querying: %j", async body => {
  expect((await POST(request(body))).status).toBe(400);
  expect(query).not.toHaveBeenCalled();
});
it.each(["missing", "disabled", "password"])("rejects %s accounts without creating a session", async reason => {
  query.mockResolvedValue({ rows: reason === "missing" ? [] : [{ ...account, disabled_at: reason === "disabled" ? new Date() : null }] });
...
it("requires explicit valid usernames when creating accounts", () => {
  ...
  expect(createUserSchema.parse({ username: " Alice_1 ", email: "user@example.com" }).username).toBe("alice_1");
});
