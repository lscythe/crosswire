...
const request = (value = key) =>
  new Request("http://localhost/api/gateway-test", {
    method: "POST",
...
beforeEach(() => {
  ...
  vi.mocked(currentUser).mockResolvedValue({ id: "member" } as never);
});
...
it("checks only accessible default providers and redacts health failures", async () => {
  ...
  expect(body.providers.map((p: { status: string }) => p.status)).toEqual([
    "healthy",
    "degraded",
    "unreachable",
  ]);
  ...
  for (const clause of [
    ...
    "cfg.is_default",
    "c.enabled",
    "c.owner_user_id = $1 OR c.visibility = 'public'",
  ])
