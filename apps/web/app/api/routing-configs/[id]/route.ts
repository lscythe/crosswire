...
async function changeDefaultOrDelete(id: string, userId: string, remove: boolean) {
  return withTransaction(async (client) => {
    ...
    if (remove) await client.query("DELETE FROM routing_configs WHERE id = $1", [id]);
      ...
      await client.query(
        "UPDATE routing_configs SET is_default = true, updated_at = now() WHERE id = $1",
        [id],
      );
    }
    await client.query(
      "INSERT INTO audit_events(actor_user_id, action, resource_type, resource_id) VALUES ($1, $2, 'routing_config', $3)",
      [userId, remove ? "routing_config.deleted" : "routing_config.default_changed", id],
    );
    return true;
  });
}
...
export async function PATCH(request: Request, context: Context) {
  ...
  return (await changeDefaultOrDelete(id, user.id, false))
...
export async function DELETE(_request: Request, context: Context) {
  ...
  if (!z.string().uuid().safeParse(id).success)
    return NextResponse.json({ error: "not found" }, { status: 404 });
  return (await changeDefaultOrDelete(id, user.id, true))
