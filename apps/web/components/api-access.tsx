...
export function ApiAccess({
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSecret({ id: body.id, key: body.key });
      form.reset();
      router.refresh();
    } catch (cause) {
  ...
  async function testRequest() {
    ...
    try {
      const response = await fetch("/api/gateway-test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ key: testKey || secret?.key }),
      });
      const body = await response.json();
      if (!response.ok || !body.ok)
        throw new Error(body.error ?? `Gateway returned HTTP ${body.status}`);
      setTestStatus(`Gateway reachable. ${body.modelCount} aliases · ${body.latencyMs}ms.`);
    } catch (cause) {
      setTestStatus(cause instanceof Error ? cause.message : "Gateway test failed");
    } finally {
      setPending("");
      router.refresh();
    }
  ...
  async function revoke(key: KeyView) {
    if (!window.confirm(`Revoke ${key.name}? Clients using this key will lose access immediately.`))
      return;
    setPending(key.id);
    setError("");
    setMessage("");
  ...
  return (
    <>
      <Card className="connection-card">
        ...
        <Button variant="secondary" type="submit" onPress={() => copy(baseUrl)}>
          Copy URL
        </Button>
        <p>
          Use a personal Crosswire key as your client’s API key. Provider credentials stay on the
          server.
        </p>
        ...
        {models.length ? (
          ...
          <p>No models available. Set a default routing config with an enabled connection.</p>
        )}
        <p>
          GET /v1/models returns your available aliases. Set CROSSWIRE_API_KEY in your terminal
          before running:
        </p>
        {exampleKind === "python" && (
          <p>
            Install: <code>pip install openai</code>
          </p>
        )}
        {exampleKind === "node" && (
          <p>
            Install: <code>npm install openai</code>. Save as <code>example.mjs</code>.
          </p>
        )}
        <div className="connection-actions">
          <Button
            variant="secondary"
            type="button"
            aria-pressed={exampleKind === "curl"}
            onPress={() => setExampleKind("curl")}
          >
            curl
          </Button>
          <Button
            variant="secondary"
            type="button"
            aria-pressed={exampleKind === "python"}
            onPress={() => setExampleKind("python")}
          >
            Python
          </Button>
          <Button
            variant="secondary"
            type="button"
            aria-pressed={exampleKind === "node"}
            onPress={() => setExampleKind("node")}
          >
            Node
          </Button>
        </div>
        <pre>{example}</pre>
        <Button variant="secondary" type="submit" onPress={() => copy(example)}>
          Copy example
        </Button>
        <label className="field">
          ...
          <Input
            ...
            placeholder="Paste a Crosswire key"
            autoComplete="off"
          />
        </label>
        <Button variant="secondary" type="submit" onPress={testRequest} isDisabled={!!pending}>
          {pending === "test" ? "Testing..." : "Test request"}
        </Button>
        {testStatus && <p role="status">{testStatus}</p>}
        <h3>Provider health</h3>
        <Button variant="secondary" type="submit" onPress={checkHealth} isDisabled={!!pending}>
          {pending === "health" ? "Checking..." : "Check health"}
        </Button>
        {health?.length === 0 && <p>No enabled providers in your default config.</p>}
        {health && (
          <ul>
            {health.map((provider) => (
              <li key={provider.id}>
                {provider.name} · {provider.status} · {provider.latencyMs}ms · HTTP{" "}
                {provider.httpStatus}
              </li>
            ))}
          </ul>
      ...
      <Card className="connection-card">
        ...
        <form onSubmit={create}>
          <label className="field">
            Key name
            <Input name="name" required maxLength={80} disabled={!!pending || !!secret} />
          </label>
          <Button
            variant="primary"
            type="submit"
            className="primary"
            isDisabled={!!pending || !!secret}
          >
        ...
        {secret && (
          <div>
            ...
            <label className="field">
              New API key
              <Input readOnly autoComplete="off" value={secret.key} />
            </label>
            <div className="connection-actions">
              <Button variant="secondary" type="submit" onPress={() => copy(secret.key)}>
                Copy key
              </Button>
              <Button variant="secondary" type="submit" onPress={() => setSecret(null)}>
                Dismiss key
              </Button>
            </div>
          </div>
        ...
        <ul>
          {keys.map((key) => (
            <li key={key.id}>
              ...
              {key.revoked_at ? "Revoked" : "Active"} · Last used (updated at most once per minute):{" "}
