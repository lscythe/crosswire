      ...
      const response = await fetch(
        connection ? `/api/providers/${connection.id}` : "/api/providers",
        {
          ...
          body: JSON.stringify({
            ...
            requestsPerDay: Number(form.get("requestsPerDay")),
          }),
        },
      );
    ...
    <form onSubmit={submit}>
      <fieldset disabled={pending} className="connection-fields">
        <label className="field">
          Name
          <Input name="name" required maxLength={100} defaultValue={connection?.name} />
        </label>
        <label className="field">
          Base URL
          <Input
        ...
        <label className="field">
          {connection ? "Replacement API key (optional)" : "API key"}
          <Input
            name="apiKey"
            type="password"
        ...
        <label className="field">
          Visibility
          <select name="visibility" defaultValue={connection?.visibility ?? "private"}>
        ...
        <label className="field">
          ...
          <Input
            ...
            min={1}
            max={10000}
            defaultValue={connection?.requests_per_minute ?? 60}
          />
        </label>
        <label className="field">
          ...
          <Input
            ...
            min={1}
            max={1000000}
            defaultValue={connection?.requests_per_day ?? 1000}
          />
        </label>
