...
test("admin creates a user who must replace the temporary password", async ({ page, browser }) => {
  ...
  expect(
    (
      await page.request.post("/api/users", {
        data: { username: email.split("@")[0].toUpperCase(), email: `other-${email}` },
      })
    ).status(),
  ).toBe(409);
  expect(
    (await page.request.post("/api/users", { data: { email: "invalid", role: "root" } })).status(),
  ).toBe(400);
  expect(
    (
      await page.request.post("/api/invitations", {
        data: { username: email.split("@")[0], email },
      })
    ).status(),
  ).toBe(404);
  ...
  try {
    ...
    try {
      ...
      expect(
        (
          await context.request.post(`${base}/api/users`, { data: { email: "denied@example.com" } })
        ).status(),
      ).toBe(401);
      for (const data of [
        { password: "short", confirmPassword: "short" },
        { password, confirmPassword: "mismatch" },
        { password: temporaryPassword, confirmPassword: temporaryPassword },
      ]) {
        expect(
          (await context.request.post(`${base}/api/auth/change-password`, { data })).status(),
        ).toBe(400);
      }
      ...
      expect(
        (
          await oldSession.request.post(`${base}/api/auth/change-password`, {
      ...
      expect(
        (
          await context.request.post(`${base}/api/auth/login`, {
            data: { username: email.split("@")[0], password: temporaryPassword },
          })
        ).status(),
      ).toBe(401);
      await member.getByLabel("Username", { exact: true }).fill(email.split("@")[0].toUpperCase());
