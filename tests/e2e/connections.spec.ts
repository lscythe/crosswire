...
test("connection management preserves secrets and enforces ownership", async ({ page, browser }) => {
  ...
  try {
    ...
    connectionId = connections.connections.find((connection: { name: string }) => connection.name === name).id;
    ...
    expect(JSON.stringify(await (await member.request.get(`${base}/api/connections`)).json())).not.toContain(name);
    ...
    await expect(card.getByText("Connection test passed. 1 models returned.", { exact: true })).toBeVisible();
    await card.getByLabel("Replacement API key (optional)").fill("wrong-test-secret");
    await card.getByRole("button", { name: "Save changes" }).click();
    await expect(card.getByLabel("Replacement API key (optional)")).toHaveValue("");
    await card.getByRole("button", { name: "Test connection", exact: true }).click();
    await expect(card.getByText("Connection test failed (HTTP 401). Check the URL and credentials.", { exact: true })).toBeVisible();
    ...
    expect((await page.request.post(`/api/connections/${connectionId}/probe`, { data: { model: "fixture-model" } })).status()).toBe(404);
    ...
    const sharedCard = sharedPage.getByRole("article").filter({ has: sharedPage.getByRole("heading", { name, exact: true }) });
    ...
    expect((await member.request.patch(`${base}/api/connections/${connectionId}`, { data: { enabled: false } })).status()).toBe(404);
    expect((await member.request.post(`${base}/api/connections/${connectionId}`)).status()).toBe(404);
    ...
    await expect(sharedCard.getByRole("heading", { name: "chat: passed", exact: true })).toBeVisible();
    await sharedPage.reload();
    await expect(sharedCard.getByText("Identity confidence: medium", { exact: true })).toBeVisible();
    await page.reload();
    ...
    const owned = await member.request.post(`${base}/api/connections`, { data: { name: `${name} member`, baseUrl: process.env.TEST_PROVIDER_URL, apiKey: "connection-test-secret", visibility: "private" } });
    const own = await owned.json();
    try {
      expect((await member.request.patch(`${base}/api/connections/${own.id}`, { data: { name: `${name} renamed` } })).status()).toBe(200);
      expect((await page.request.patch(`/api/connections/${own.id}`, { data: { enabled: false } })).status()).toBe(200);
    } finally {
