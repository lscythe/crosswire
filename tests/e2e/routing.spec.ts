...
test("personal routing CRUD, ordered fallback, isolation and request evidence", async ({
  page,
  browser,
}) => {
  ...
  try {
      ...
      expect(
        (
          await context.request.post(`${base}/api/auth/change-password`, {
            data: {
              password: "routing-member-password",
              confirmPassword: "routing-member-password",
            },
          })
        ).status(),
      ).toBe(200);
      await context.request.post(`${base}/api/auth/login`, {
        data: { username: email.split("@")[0], password: "routing-member-password" },
      });
    }
    const keys = [];
    for (const context of [owner, other])
      keys.push(
        (
          await (
            await context.request.post(`${base}/api/keys`, { data: { name: "routing check" } })
          ).json()
        ).key,
      );
    const names = ["Failing provider", "Backup provider"];
    for (let i = 0; i < 2; i++) {
      const connection = await owner.request.post(`${base}/api/connections`, {
        data: {
          name: names[i],
          baseUrl: i === 0 ? providerUrl.replace("/v1", "/fail/v1") : providerUrl,
    ...
    expect(
      await routingPage.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth),
    ).toBe(true);
    await routingPage.screenshot({ path: "test-results/routing-mobile.png", fullPage: true });
    const configs = async () =>
      (await (await owner.request.get(`${base}/api/routing-configs`)).json()).configs;
    let cfg = (await configs())[0];
    configIds.push(cfg.id);
    const chat = (key: string, requestId = randomUUID()) =>
      owner.request.post(`${gateway}/v1/chat/completions`, {
        headers: { Authorization: `Bearer ${key}`, "X-Request-ID": requestId },
        data: { model: "team-chat", messages: [{ role: "user", content: "OK" }] },
      });
    ...
    const logs = async () =>
      (await (await owner.request.get(`${base}/api/request-logs`)).json()).requests;
    await expect
      .poll(
        async () =>
          (await logs()).find((log: { request_id: string }) => log.request_id === requestId)
            ?.attempts.length,
      )
      .toBe(2);
    ...
    expect(log.attempts.map((item: { reason: string }) => item.reason)).toEqual([
    ...
    await expect(
      routingPage.getByText("Served by: Backup provider", { exact: true }),
    ).toBeVisible();
    await routingPage.getByText("Routing attempts (2)", { exact: true }).click();
    await expect(
      routingPage.getByText(/Failing provider · primary-model · HTTP 503/),
    ).toBeVisible();
    await routingPage.goto(`${base}/routing`);
    ...
    await expect
      .poll(async () => (await configs())[0].routes[0].connectionId)
    ...
    await expect
      .poll(
        async () =>
          (await logs()).find((item: { request_id: string }) => item.request_id === directId)
            ?.attempts.length,
      )
      .toBe(1);
    cfg = (await configs())[0];
    ...
    const bad = {
      name: "bad",
      isDefault: true,
      routes: [{ ...cfg.routes[0], connectionId: randomUUID() }],
    };
    expect((await owner.request.post(`${base}/api/routing-configs`, { data: bad })).status()).toBe(
      400,
    );
    expect(
      (await owner.request.put(`${base}/api/routing-configs/${cfg.id}`, { data: bad })).status(),
    ).toBe(400);
    expect((await configs())[0].name).toBe("Team routing");
    expect((await configs())[0].is_default).toBe(true);
    await owner.request.patch(`${base}/api/connections/${connectionIds[1]}`, {
      data: { enabled: false },
    });
    expect(
      (
        await owner.request.put(`${base}/api/routing-configs/${cfg.id}`, {
          data: { name: cfg.name, isDefault: true, routes: cfg.routes },
        })
      ).status(),
    ).toBe(400);
    await owner.request.patch(`${base}/api/connections/${connectionIds[1]}`, {
    ...
    for (const method of ["patch", "delete", "put"] as const) {
      expect(
        (
          await other.request[method](`${base}/api/routing-configs/${cfg.id}`, {
            data: method === "put" ? { name: "stolen", routes: cfg.routes } : { isDefault: true },
          })
        ).status(),
      ).toBe(404);
    }
    const second = await (
      await owner.request.post(`${base}/api/routing-configs`, {
        data: { name: "Alternative", routes: cfg.routes },
      })
    ).json();
    configIds.push(second.id);
    await routingPage.reload();
    await routingPage
      .getByRole("article", { name: "Alternative", exact: true })
      .getByRole("button", { name: "Make default" })
      .click();
    await expect
      .poll(async () =>
        (await configs())
          .filter((item: { is_default: boolean }) => item.is_default)
    ...
    const switches = await Promise.all(
      configIds.map((id) =>
        owner.request.patch(`${base}/api/routing-configs/${id}`, { data: { isDefault: true } }),
      ),
    );
    expect(switches.map((response) => response.status())).toEqual([200, 200]);
    expect(
      (await configs()).filter((item: { is_default: boolean }) => item.is_default),
    ).toHaveLength(1);
    await owner.request.patch(`${base}/api/routing-configs/${cfg.id}`, {
      data: { isDefault: true },
    });
    // Stop on 4xx; never use the backup for an authentication error.
    await owner.request.patch(`${base}/api/connections/${connectionIds[1]}`, {
      data: { baseUrl: providerUrl.replace("/v1", "/deny/v1") },
    });
    expect((await chat(keys[0])).status()).toBe(401);
    // Transport failure falls back; all failures produce an explainable 502.
    await owner.request.patch(`${base}/api/connections/${connectionIds[1]}`, {
      data: { baseUrl: providerUrl.replace("/v1", "/drop/v1") },
    });
    ...
    await expect
      .poll(
        async () =>
          (await logs()).find((item: { request_id: string }) => item.request_id === failedId)
            ?.attempts[0].reason,
      )
      .toBe("transport error");
    // A formerly public connection is no longer routable by other users.
    const foreign = await (
      await other.request.post(`${base}/api/routing-configs`, {
        data: { name: "Shared", isDefault: true, routes: [cfg.routes[0]] },
      })
    ).json();
    await owner.request.patch(`${base}/api/connections/${connectionIds[1]}`, {
    ...
    for (const id of configIds)
      await owner.request
        .delete(`${base}/api/routing-configs/${id}`, { timeout: 2000 })
        .catch(() => {});
    for (const id of connectionIds)
      await owner.request
        .delete(`${base}/api/connections/${id}`, { timeout: 2000 })
        .catch(() => {});
    await owner.close();
    await other.close();
  }
