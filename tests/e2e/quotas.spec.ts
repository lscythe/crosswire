...
test("public quotas are atomic across keys, isolated per user, visible to owners, and allow fallback", async ({ request, browser }) => {
  ...
  try {
    ...
    const connection = await owner.post(`${base}/api/connections`, { data: { name: "Quota provider", baseUrl: process.env.TEST_PROVIDER_URL, apiKey: "connection-test-secret", visibility: "public", requestsPerMinute: 1, requestsPerDay: 10 } });
    expect(connection.status()).toBe(201);
    const id = (await connection.json()).id; connectionIds.push(id);
    const route = { modelAlias: "quota-chat", connectionId: id, upstreamModel: "fixture-model", priority: 0 };
    for (const context of contexts) {
      const result = await context.request.post(`${base}/api/routing-configs`, { data: { name: "Quota routing", isDefault: true, routes: [route] } });
      expect(result.status()).toBe(201); configIds.push((await result.json()).id);
    }
    keys.push({ ...await (await owner.post(`${base}/api/keys`, { data: { name: "Second key same user" } })).json(), user: 0 });
    const chat = (key: string) => request.post(`${gateway}/v1/chat/completions`, { headers: { Authorization: `Bearer ${key}` }, data: { model: "quota-chat", messages: [{ role: "user", content: "OK" }] } });
    const concurrent = await Promise.all([chat(keys[0].key), chat(keys[2].key)]);
    expect(concurrent.map(r => r.status()).sort()).toEqual([200, 429]);
    expect(Number(concurrent.find(r => r.status() === 429)!.headers()["retry-after"])).toBeGreaterThan(0);
    expect((await chat(keys[1].key)).status()).toBe(200);
    expect((await other.patch(`${base}/api/connections/${id}`, { data: { requestsPerDay: 100 } })).status()).toBe(404);
    expect((await owner.patch(`${base}/api/connections/${id}`, { data: { requestsPerMinute: 100, requestsPerDay: 2 } })).status()).toBe(200);
    expect((await chat(keys[0].key)).status()).toBe(200);
    expect((await chat(keys[0].key)).status()).toBe(429);
    expect((await owner.post(`${base}/api/connections/${id}/probe`, { data: { model: "fixture-model" } })).status()).toBe(429);
    const ownerPage = await contexts[0].newPage(); await ownerPage.goto(`${base}/connections`);
    ...
    const otherPage = await contexts[1].newPage(); await otherPage.goto(`${base}/connections`);
    ...
    const backup = await (await owner.post(`${base}/api/connections`, { data: { name: "Private quota backup", baseUrl: process.env.TEST_PROVIDER_URL, apiKey: "connection-test-secret" } })).json(); connectionIds.push(backup.id);
    await owner.put(`${base}/api/routing-configs/${configIds[0]}`, { data: { name: "Quota routing", isDefault: true, routes: [route, { ...route, connectionId: backup.id, priority: 1 }] } });
    expect((await chat(keys[0].key)).status()).toBe(200);
    const logs = (await (await owner.get(`${base}/api/request-logs`)).json()).requests;
    expect(logs[0].attempts.map((attempt: { reason: string }) => attempt.reason)).toEqual(["public connection quota exceeded", "response served"]);
  } finally {
    for (const key of keys) await contexts[key.user].request.delete(`${base}/api/keys/${key.id}`);
    for (const [index, id] of configIds.entries()) await contexts[index].request.delete(`${base}/api/routing-configs/${id}`);
    for (const id of connectionIds) await contexts[0].request.delete(`${base}/api/connections/${id}`);
